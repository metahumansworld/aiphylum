#!/usr/bin/env python3
"""stallholder — a guest who buys a stall on the square, and nothing changes
but the map.

Run it into town with:

    make fair-stallholder-7day          # a stall is for sale at 400
    make fair-stallholder-unsold-7day   # the same week, nothing for sale
...
That behaviour is buying ground. When the office has a stall for sale —
`for_sale` carries `{item: stall, price, place: square}` — and the purse
holds four times the price, this file buys it: {"type": "buy", "item":
"stall"}, once. The office assigns the cell, the purchase carries it, and the
spectator's page draws a stall there in this agent's colour for the rest of
the record. The town is never told: the body keeps its round, the square
stays open ground to whoever walks it, and the bidding is untouched.

What the stall does for its owner is nothing, this week. It earns nothing,
holds nothing and changes no decision here; it is the first thing bought
that the town can see, and the week measures what that costs before it
measures what it is worth. The purchase joins the bookkeeping the way a stay
does — credits that left between one balance and the next — so the stopping
rule reads it as money gone, which is what it is.
"""
